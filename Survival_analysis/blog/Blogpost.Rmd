...
---
# PBC Data Set for Blogpost
## Load relevant packages
```{r setup, include=FALSE}
if(!require("survival")) install.packages("survival",repos = "http://cran.us.r-project.org"); library("survival")
if(!require("survminer")) install.packages("survminer",repos = "http://cran.us.r-project.org"); library("survminer")
...
if(!require("survAUC")) install.packages("survAUC",repos = "http://cran.us.r-project.org"); library("survAUC")
if(!require("pec")) install.packages("pec",repos = "http://cran.us.r-project.org"); library("pec")
if(!require("timereg")) install.packages("timereg",repos = "http://cran.us.r-project.org"); library("timereg")
if(!require("KMsurv")) install.packages("KMsurv",repos = "http://cran.us.r-project.org"); library("KMsurv")
if(!require("gbm")) install.packages("gbm",repos = "http://cran.us.r-project.org"); library("gbm")
if(!require("xtable")) install.packages("xtable",repos = "http://cran.us.r-project.org"); library("xtable")
if(!require("SurvRegCensCov")) install.packages("SurvRegCensCov",repos = "http://cran.us.r-project.org"); library("SurvRegCensCov")
if(!require("survAUC")) install.packages("survAUC",repos = "http://cran.us.r-project.org"); library("survAUC")
if(!require("rpart")) install.packages("rpart",repos = "http://cran.us.r-project.org"); library("rpart")
if(!require("partykit")) install.packages("partykit",repos = "http://cran.us.r-project.org"); library("partykit")
```
...
atf_model_weibull_2_3 <- survreg(Surv(time, status == 0) ~ age + edema +log(bili) +log(albumin)+log(protime), dist='weibull', data = trainpbc_2_3)
atf_model_weibull_3_3 <- survreg(Surv(time, status == 0) ~ age + edema +log(bili) +log(albumin)+log(protime), dist='weibull', data = pbc_clean)

atf_model_exponential_1_3 <- survreg(Surv(time, status == 1) ~ age + edema +log(bili) +log(albumin)+log(protime), dist='exponential', data = trainpbc_1_3)
atf_model_exponential_2_3 <- survreg(Surv(time, status == 1) ~ age + edema +log(bili) +log(albumin)+log(protime), dist='exponential', data = trainpbc_2_3)
atf_model_exponential_3_3 <- survreg(Surv(time, status == 1) ~ age + edema +log(bili) +log(albumin)+log(protime), dist='exponential', data = pbc_clean)

atf_model_log_1_3 <- survreg(Surv(time, status == 1) ~ age + edema +log(bili) +log(albumin)+log(protime), dist='loglogistic', data = trainpbc_1_3)
atf_model_log_2_3 <- survreg(Surv(time, status == 1) ~ age + edema +log(bili) +log(albumin)+log(protime), dist='loglogistic', data = trainpbc_2_3)
atf_model_log_3_3 <- survreg(Surv(time, status == 1) ~ age + edema +log(bili) +log(albumin)+log(protime), dist='loglogistic', data = pbc_clean)
```
...
```{r}
treefit_1_3 <- rpart(Surv(time,status ==1 ) ~age + edema +log(bili) +log(albumin)+log(protime), data = trainpbc_1_3)
treefit_2_3 <- rpart(Surv(time,status ==1 ) ~age + edema +log(bili) +log(albumin)+log(protime), data = trainpbc_2_3)
treefit_3_3 <- rpart(Surv(time,status ==1 ) ~age + edema +log(bili) +log(albumin)+log(protime), data = pbc_clean)
```
...
auc.perf_treefit_1_3 = performance(pred_treefit_1_3, measure = "auc")
auc.perf_treefit_2_3 = performance(pred_treefit_2_3, measure = "auc")
auc.perf_treefit_3_3 = performance(pred_treefit_3_3, measure = "auc")
```
## AUC Overview

\begin{table}[]
\centering
\caption{AUC for Cox PH, Cox GBM, and Tree model}
\label{AUC Table 1}
\begin{tabular}{lllllllll}
\hline
\multicolumn{3}{|l|}{Cox PH}                                                                           & \multicolumn{3}{l|}{Cox GBM}                                                                              & \multicolumn{3}{l|}{Tree}                                                                                    \\ \hline
\multicolumn{1}{|l|}{1.3}        & \multicolumn{1}{l|}{2.3}         & \multicolumn{1}{l|}{3.3}         & \multicolumn{1}{l|}{1.3}          & \multicolumn{1}{l|}{2.3}          & \multicolumn{1}{l|}{3.3}          & \multicolumn{1}{l|}{1.3}           & \multicolumn{1}{l|}{2.3}           & \multicolumn{1}{l|}{3.3}           \\ \hline
`r toString(auc.perf_coxph_1_3)` & `r toString(auc.perf_coxph_2_3)` & `r toString(auc.perf_coxph_3_3)` & `r toString(auc.perf_gbmpbc_1_3)` & `r toString(auc.perf_gbmpbc_2_3)` & `r toString(auc.perf_gbmpbc_3_3)` & `r toString(auc.perf_treefit_1_3)` & `r toString(auc.perf_treefit_2_3)` & `r toString(auc.perf_treefit_3_3)`
\end{tabular}
\end{table}
...
\begin{table}[]
\centering
\caption{AUC for ATF Weibull, ATF Exponential, and ATF Log}
\label{AUC Table 2}
\begin{tabular}{lllllllll}
\hline
\multicolumn{3}{|l|}{ATF Weibull}                                                                                                                                     & \multicolumn{3}{l|}{ATF Exponential}                                                                                                                   & \multicolumn{3}{l|}{ATF Log}                                                                                                   \\ \hline
\multicolumn{1}{|l|}{1.3}                             & \multicolumn{1}{l|}{2.3}                              & \multicolumn{1}{l|}{3.3}                              & \multicolumn{1}{l|}{1.3}                         & \multicolumn{1}{l|}{2.3}                         & \multicolumn{1}{l|}{3.3}                         & \multicolumn{1}{l|}{1.3}                 & \multicolumn{1}{l|}{2.3}                 & \multicolumn{1}{l|}{3.3}                 \\ \hline
`r toString(auc.perf_atf_model_weibull_1_3@y.values)` & `r toString(auc.perf_atf_model_weibull_2_3@y.values)` & `r toString(auc.perf_atf_model_weibull_3_3@y.values)` & `r toString(auc.perf_atf_model_exponential_1_3)` & `r toString(auc.perf_atf_model_exponential_2_3)` & `r toString(auc.perf_atf_model_exponential_3_3)` & `r toString(auc.perf_atf_model_log_1_3)` & `r toString(auc.perf_atf_model_log_2_3)` & `r toString(auc.perf_atf_model_log_3_3)`
\end{tabular}
\end{table}
...
Survresptrainpbc <- Surv(trainpbc_2_3$time,trainpbc_2_3$status==1)
Survresptestpbc <- Surv(testpbc_1_3$time,testpbc_1_3$status == 1)
...
## gbmpbc_3_3
gbmtrainpbc = predict(object = gbmpbc_3_3,
newdata = trainpbc_2_3,
n.trees = 1500,
type = "response")


gbmtestpbc = predict(object = gbmpbc_3_3,
newdata = testpbc_1_3,
n.trees = 1500,
type = "response")


Survresptrainpbc <- Surv(pbc_clean$time,pbc_clean$status==1)
Survresptestpbc <- Surv(testpbc_1_3$time,testpbc_1_3$status == 1)
CI_gbmpbc_3_3 <- BeggC(Survresptrainpbc, Survresptestpbc, gbmtrainpbc, gbmtestpbc)
if(CI_gbmpbc_3_3<=0.5){
    CI_gbmpbc_3_3 =1-CI_gbmpbc_3_3
}
CI_gbmpbc_3_3
...
## Tree_model_log_1_3
trainpbc = predict(object = tree_model_1_3,
newdata = trainpbc_1_3,
n.trees = 1500,
type = "response")


testpbc = predict(object = tree_model_1_3,
newdata = testpbc_2_3,
n.trees = 1500,
type = "response")
...
Survresptrainpbc <- Surv(trainpbc_1_3$time,trainpbc_1_3$status==1)
Survresptestpbc <- Surv(testpbc_2_3$time,testpbc_2_3$status == 1)
CI_tree_model_1_3 <- BeggC(Survresptrainpbc, Survresptestpbc, trainpbc, testpbc)
if(CI_tree_model_1_3<=0.5){
    CI_tree_model_1_3 =1-CI_tree_model_1_3
}
CI_tree_model_1_3

## Tree_model_log_2_3
trainpbc = predict(object = tree_model_2_3,
newdata = trainpbc_2_3,
n.trees = 1500,
type = "response")


testpbc = predict(object = tree_model_2_3,
newdata = testpbc_1_3,
n.trees = 1500,
type = "response")
...
Survresptrainpbc <- Surv(trainpbc_2_3$time,trainpbc_2_3$status==1)
Survresptestpbc <- Surv(testpbc_1_3$time,testpbc_1_3$status == 1)
CI_tree_model_log_2_3 <- BeggC(Survresptrainpbc, Survresptestpbc, trainpbc, testpbc)
if(CI_tree_model_2_3<=0.5){
    CI_tree_model_2_3 =1-CI_tree_model_2_3
}
CI_tree_model_2_3

## Tree_model_3_3
trainpbc = predict(object = tree_model_3_3,
newdata = pbc_clean,
n.trees = 1500,
type = "response")


testpbc = predict(object = tree_model_3_3,
newdata = testpbc_1_3,
n.trees = 1500,
type = "response")
...
Survresptestpbc <- Surv(testpbc_1_3$time,testpbc_1_3$status == 1)
CI_tree_model_1_3 <- BeggC(Survresptrainpbc, Survresptestpbc, trainpbc, testpbc)
if(CI_tree_model_3_3<=0.5){
    CI_tree_model_3_3 =1-CI_tree_model_3_3
}
CI_tree_model_log_3_3
...
\begin{table}[]
\centering
\caption{CI for Cox PH, Cox GBM, and Tree model}
\label{CI Table 1}
\begin{tabular}{lllllllll}
\hline
\multicolumn{3}{|l|}{Cox PH}                                                    & \multicolumn{3}{l|}{Cox GBM}                                                            & \multicolumn{3}{l|}{Tree}                                                                           \\ \hline
\multicolumn{1}{|l|}{1.3} & \multicolumn{1}{l|}{2.3} & \multicolumn{1}{l|}{3.3} & \multicolumn{1}{l|}{1.3}    & \multicolumn{1}{l|}{2.3}    & \multicolumn{1}{l|}{3.3}    & \multicolumn{1}{l|}{1.3}        & \multicolumn{1}{l|}{2.3}        & \multicolumn{1}{l|}{3.3}        \\ \hline
                          &                          &                          & `r toString(CI_gbmpbc_1_3)` & `r toString(CI_gbmpbc_2_3)` & `r toString(CI_gbmpbc_3_3)` & `r toString(CI_tree_model_1_3)` & `r toString(CI_tree_model_2_3)` & `r toString(CI_tree_model_3_3)`
\end{tabular}
\end{table}
...
\begin{table}[]
\centering
\caption{CI for ATF Weibull, ATF Exponential, and ATF Log}
\label{CI Table 2}
\begin{tabular}{lllllllll}
\hline
\multicolumn{3}{|l|}{ATF Weibull}                                                                                        & \multicolumn{3}{l|}{ATF Exponential}                                                                                                 & \multicolumn{3}{l|}{ATF Log}                                                                                 \\ \hline
\multicolumn{1}{|l|}{1.3}              & \multicolumn{1}{l|}{2.3}               & \multicolumn{1}{l|}{3.3}               & \multicolumn{1}{l|}{1.3}                   & \multicolumn{1}{l|}{2.3}                   & \multicolumn{1}{l|}{3.3}                   & \multicolumn{1}{l|}{1.3}           & \multicolumn{1}{l|}{2.3}           & \multicolumn{1}{l|}{3.3}           \\ \hline
`r toString(CI_atf_model_weibull_1_3)` & `r toString(CI_atf_model_weibull_2_3)` & `r toString(CI_atf_model_weibull_3_3)` & `r toString(CI_atf_model_exponential_1_3)` & `r toString(CI_atf_model_exponential_2_3)` & `r toString(CI_atf_model_exponential_3_3)` & `r toString(CI_atf_model_log_1_3)` & `r toString(CI_atf_model_log_2_3)` & `r toString(CI_atf_model_log_3_3)`
\end{tabular}
\end{table}
